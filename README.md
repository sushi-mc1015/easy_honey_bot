...
目的
このプログラムは、管理者ログイン画面に見せかけた教育用のHTTPハニーボットである。
ブラウザからアクセスするとログイン画面を表示し、入力されたユーザー名、
パスワード、接続元IPアドレス、ポート番号、アクセス先をログへ記録する。

実際の認証処理は行わず、入力内容に関係なく「Login Failed」を返す。
...
---起動方法---
ターミナルまたはコマンドプロンプトでweb_honeybotフォルダへ移動。
